...
class BasePredictor:
    ...
    def preprocess(self, im):
        ...
        if not_tensor:
            ...
            im_sam=im.transpose((0, 3, 1, 2))
            im = im[..., ::-1].transpose((0, 3, 1, 2))  # BGR to RGB, BHWC to BCHW, (n, 3, h, w)
            im = np.ascontiguousarray(im)  # contiguous
            im_sam=np.ascontiguousarray(im_sam)
            im = torch.from_numpy(im)
            im_sam=torch.from_numpy(im_sam)
            self.img = im_sam.to(self.device)
    ...
    def stream_inference(self, source=None, model=None, *args, **kwargs):
        ...
        with self._lock:  # for thread-safe inference
            ...
            for batch in self.dataset:
                ...
                if self.args.verbose:
                   LOGGER.info(f"{s}{profilers[0].dt * 1E3:.1f}ms,{profilers[1].dt * 1E3:.1f}ms,{profilers[2].dt * 1E3:.1f}ms")
